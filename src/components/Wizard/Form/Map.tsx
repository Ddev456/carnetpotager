...
export const Map = ({handleClimateIsDefined}: {handleClimateIsDefined: Dispatch<SetStateAction<boolean>>}) => {
    ...
    const climateLs: ClimateType = useReadLocalStorage("climate") ?? {};
    // const [climate, setClimate] = useState({});
    const { addToClimate } = useClimate();
    useEffect(()=>{
      setClimate(climateLs);
    },[clicked])
    
    // const addToClimate = (climateLs: string) => {
    //   setClimate(climateLs)
    // }
  
    // useEffect(() => {
    //   if (Object.values(climate).length === 0) return
  ...
  return (
    <div className="map">
        <span className="font-bold text-lg">Mon climat : { climate.climate && climate.climate !== -1 ? climate.name : clicked.name }</span>
        { climate.dept && climate.dept.length !== 0 ? getClimateFromDept(climate.dept) : clicked.id}
